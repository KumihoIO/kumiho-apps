...
### 🧠 Cognitive Memory Auditing (AI Agent Transparency)

- Browse recorded AI agent memories as **Items** with full **Revision history** (conversations, decisions, facts, tool executions)
- Visualize provenance and dependencies in the graph to understand *why* a conclusion was reached
- Review and curate memories for auditability, debugging, and long-term reuse

Kumiho Browser (`kumiho-browser`) makes this review process human-auditable: you can follow lineage edges to see what evidence a memory was derived from, and what other conclusions depend on it.

In practice, this looks like browsing a dedicated `CognitiveMemory` project the same way you browse assets: navigate to spaces like `user/preferences`, select an item, and inspect the details pane for revision history, tags (e.g. `latest`), the stable `kref://...` identifier, schema-versioned metadata, and any attached artifacts.
